...
| 功能 | 說明 |
|---|---|
| 總覽 | 資產走勢、現金流走勢，含月對月/年對年比較。總資產／總負債／淨資產**依「設定 › 帳戶分類」歸類**：分類名稱含「資產」計入總資產、含「負債」計入總負債（取絕對值後轉負），分類名稱含「忽視／忽略／不計入／排除」者視為刻意排除（不跳提醒），其餘分類與未分類帳戶不計入且會提醒，畫面會提示未分類的帳戶筆數與金額。淨資產＝總資產＋總負債 |
| 收支明細 | 總收支/收入/支出明細查詢，支援關鍵字搜尋、月份篩選、行內編輯、排除（軟刪除）|
| 分類分析 | 依收入/支出、月/年粒度做分類統計，支援下鑽查看單筆明細 |
| 專案管理 | 財務專案追蹤，包含三個獨立子系統，摘要列表的**達成率**＝上月實際資產 ÷ 上月預期資產（各自取該專案資料實際存在的最新月份）；專案可勾選「將現金流計入上月實際資產」，計入時只加總交易日期 ≦ 資產流最新綁定月份的命中明細累計淨額，列表會標註「（含現金流）」。收支對比圖表為每個專案一張小圖、各自一條 y 軸，避免不同量級的專案互相壓縮 |
| ├─ 資產流 | 追蹤專案綁定資產的**實際淨資產變化**，確認資產是否穩定成長。綁定改以彈窗操作：條列所有帳戶的勾選／資產分類／帳戶／對應餘額，勾選後可選擇「套用至版本月份」或「套用到所有月份」，未按套用不會存檔 |
...
| 集保存摺 | 上傳 › 集保存摺。每月結帳流程：逐份上傳股票庫存 PDF（用 PdfPig 依文字座標還原表格）→ 確認當月清單 → 按「結算本月」把所有庫存合併成一筆帳戶（集保／集保庫存）寫入 `FIN.BankAccount` 供資產管理。機構與帳戶由 PDF 內容辨識，不需手填。防呆：同檔案（檔案雜湊）、同月同內容（內容雜湊）一律擋下；同來源的更新版跳確認後取代；當月已結算再按會擋下並提供「重新結算」覆蓋。集保只有「餘額 × 收盤價 = 市值」，**沒有成本**，成本沿用同一檔股票先前的紀錄（第一次匯入為 0）|
| 一般設定 | 設定 › 一般設定。① 集保 PDF 開啟密碼可存起來，之後上傳免輸入（密碼經 Data Protection 加密後存於 `FIN.UserSetting`，API 只回傳「是否已設定」）；② 每月結帳日（1~28），供上傳頁的結帳檢查清單倒數 |
| 結帳檢查清單 | 上傳頁（麻布資料／集保存摺）頂端顯示當月四項待辦：麻布明細、麻布帳戶餘額、集保存摺已上傳、集保存摺已結算；已過結帳日又還有未完成項目時整塊加紅框強調。集保結算後若又上傳或刪除存摺，結算項目會退回未完成 |
...
| GET/PUT/DELETE | `/api/finance/projects/{id}/assets` | 專案資產流綁定（GET 回傳含帳戶分類 `Category`，供綁定彈窗顯示；DELETE 清除所有月份） |
| POST | `/api/finance/projects/{id}/assets/apply-all-months` | 把目前勾選的帳戶套用到所有月份（各月僅套用該月實際存在的帳戶） |
